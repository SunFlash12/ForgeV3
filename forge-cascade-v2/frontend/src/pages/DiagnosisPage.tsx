    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800">Differential Diagnosis</h1>
        </div>
      ...
      {step === 'input' && (
        <div className="space-y-6">
              ...
              {searchResults.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                  {searchResults.map((phenotype) => (
                    ...
                    >
                      <div className="font-medium text-slate-800">{phenotype.name}</div>
                      <div className="text-xs text-slate-500">{phenotype.hpo_id}</div>
              ...
              {selectedPhenotypes.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedPhenotypes.map((phenotype) => (
                    <div
                      key={phenotype.hpo_id}
                      className="flex items-center gap-2 px-3 py-2 bg-violet-50 text-violet-700 rounded-lg"
                    >
          ...
          <Card className="p-6 bg-gradient-to-br from-violet-50 to-indigo-50 border-violet-200">
            <h3 className="font-semibold text-violet-800 mb-2">About This Tool</h3>
      ...
      {step === 'processing' && (
        <Card className="p-12 text-center">
          ...
          <h2 className="text-xl font-semibold text-slate-800 mb-2">Analyzing Symptoms</h2>
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="mt-2 text-sm text-slate-500">{progress}% complete</div>
          </div>
      ...
      {step === 'results' && (
        <div className="space-y-6">
          ...
          <Card className="p-4">
            <div className="flex flex-wrap gap-2">
              <span className="text-sm font-medium text-slate-600 mr-2">Analyzed:</span>
          ...
          {results.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-slate-500">No matching diagnoses found for the provided symptoms.</p>
            </Card>
          ) : (
              ...
              {results.map((result, index) => (
                <Card
                  ...
                  className={`p-4 transition-all ${
                    expandedResult === result.disease_id ? 'ring-2 ring-violet-300' : ''
                  }`}
                >
                  <div
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="flex items-center justify-center w-8 h-8 bg-slate-100 rounded-full text-slate-600 font-semibold text-sm">
                        {index + 1}
                      </div>
                      <div>
                        <h3 className="font-semibold text-slate-800">{result.disease_name}</h3>
                        <p className="text-sm text-slate-500">{result.disease_id}</p>
                      </div>
                    </div>
      ...
      >
        {loadingDetails ? (
          <div className="py-12 text-center">
            ...
            <p className="text-slate-500 mt-2">Loading details...</p>
          </div>
        ) : (
          <div className="space-y-6">
            {/* Drug Information */}
            <div>
              ...
              {drugInfo.length === 0 ? (
                <p className="text-slate-500 text-sm">No drug associations found.</p>
              ) : (
                  ...
                  {drugInfo.map((drug) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-sm text-slate-500">{drug.relationship}</div>
                      </div>
                      {drug.evidence_level && (
            ...
            <div>
              ...
              {geneInfo.length === 0 ? (
                <p className="text-slate-500 text-sm">No gene associations found.</p>
              ) : (
                <div className="space-y-2">
                  {geneInfo.map((gene) => (
                    <div
                    ...
                    >
                      <div>
                        <div className="font-medium text-slate-800">{gene.gene_symbol}</div>
                        <div className="text-sm text-slate-500">{gene.association_type}</div>
                      </div>
                      {gene.evidence && (
