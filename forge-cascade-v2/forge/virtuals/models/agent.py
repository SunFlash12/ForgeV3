...
class AgentPersonality(BaseModel):
    ...
    description: str = Field(
        max_length=2000, description="Detailed description of the agent's purpose and capabilities"
    )
    ...
    expertise_domains: list[str] = Field(
        default_factory=list, description="Areas of expertise the agent specializes in"
    )
    response_guidelines: str = Field(
        default="", description="Guidelines for how the agent should respond to queries"
    )
    ...
    def to_game_prompt(self) -> str:
        ...
        traits_str = (
            ...
            else "helpful and professional"
        )
        domains_str = (
            ", ".join(self.expertise_domains) if self.expertise_domains else "general knowledge"
        )
...
class WorkerDefinition(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique identifier for the worker"
    )
    name: str = Field(description="Worker name for identification")
    description: str = Field(description="Description of worker's purpose and capabilities")
    function_names: list[str] = Field(
        default_factory=list, description="Names of functions this worker can execute"
    )
    state_schema: dict[str, Any] = Field(
        default_factory=dict, description="Schema for the worker's state object"
    )
    max_concurrent_tasks: int = Field(
        default=5, ge=1, description="Maximum tasks this worker can handle concurrently"
    )
...
class AgentGoals(BaseModel):
    ...
    primary_goal: str = Field(description="The agent's main objective")
    secondary_goals: list[str] = Field(
        default_factory=list, description="Additional objectives to pursue"
    )
    constraints: list[str] = Field(
        default_factory=list, description="Constraints on agent behavior"
    )
    success_metrics: list[str] = Field(
        default_factory=list, description="Metrics used to evaluate agent performance"
    )
...
class AgentMemoryConfig(BaseModel):
    ...
    enable_long_term_memory: bool = Field(
        default=True, description="Enable persistent long-term memory"
    )
    memory_retention_days: int = Field(default=365, description="Days to retain long-term memories")
    max_working_memory_items: int = Field(
        default=100, description="Maximum items in working memory"
    )
    enable_cross_platform_sync: bool = Field(
        default=True, description="Sync memory across platforms (Telegram, Twitter, etc.)"
    )
    vector_embedding_model: str = Field(
        default="text-embedding-3-small", description="Model used for memory embeddings"
    )
...
class ForgeAgentCreate(BaseModel):
    ...
    # Core Identity
    name: str = Field(
        min_length=3, max_length=64, description="Agent name (used for token symbol derivation)"
    )
    ...
    forge_overlay_id: str | None = Field(
        default=None, description="ID of the Forge overlay this agent represents"
    )
    forge_capsule_ids: list[str] = Field(
        default_factory=list, description="IDs of Forge capsules this agent has access to"
    )

    # Workers Configuration
    workers: list[WorkerDefinition] = Field(
        default_factory=list, description="Worker definitions for task execution"
    )
    ...
    enable_tokenization: bool = Field(
        default=False, description="Whether to tokenize this agent (opt-in)"
    )
    token_symbol: str | None = Field(
        default=None, max_length=10, description="Token symbol if tokenizing (e.g., 'FRGSEC')"
    )
    initial_virtual_stake: float = Field(
        default=100.0, ge=100.0, description="Initial VIRTUAL stake (minimum 100)"
    )

    # Chain Configuration
    primary_chain: str = Field(default="base", description="Primary blockchain for deployment")
...
class ForgeAgent(VirtualsBaseModel):
    ...
    game_agent_id: str | None = Field(default=None, description="ID assigned by GAME framework")
    api_access_token: str | None = Field(
        default=None, description="Current API access token (rotates)"
    )

    # Blockchain State
    primary_chain: str = Field(default="base")
    wallets: dict[str, WalletInfo] = Field(
        default_factory=dict, description="Wallets by chain (e.g., {'base': WalletInfo(...)})"
    )

    # Tokenization State
    tokenization_status: TokenizationStatus = Field(default=TokenizationStatus.NOT_TOKENIZED)
    token_info: TokenInfo | None = None

    # NFT State (ERC-721 + ERC-6551 TBA)
    nft_token_id: str | None = Field(default=None, description="Agent NFT token ID")
    token_bound_account: str | None = Field(
        default=None, description="ERC-6551 token-bound account address"
    )
    ...
    trust_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Trust score based on performance"
    )
