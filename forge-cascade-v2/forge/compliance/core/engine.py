...
class ComplianceEngine:
    ...
    async def log_event(
        ...
        # Log to structlog
        log_method = getattr(
            logger, risk_level.value if hasattr(risk_level, "value") else risk_level
        )
        log_method(
            "audit_event",
            event_id=event.id,
                        ...
                        "authority_notifications": [
                            {
                                ...
                                "jurisdiction": n.jurisdiction.value,
    ...
    async def register_ai_system(
        ...
        if self._persistence_enabled and self._repository:
            try:
                ...
                logger.error(
                    "ai_system_persistence_failed", system_id=registration.id, error=str(e)
                )
    ...
    async def log_ai_decision(
        ...
        if self._persistence_enabled and self._repository:
            try:
                ...
                logger.error(
                    "ai_decision_persistence_failed", decision_id=decision.id, error=str(e)
                )
    ...
    async def generate_compliance_report(
        ...
        dsar_metrics = {
            ...
            "pending": len(
                [d for d in period_dsars if d.status in ("received", "verified", "processing")]
            ),
        ...
        consent_metrics = {
            ...
            "active_consents": sum(
                len([c for c in consents if c.is_valid]) for consents in self._consents.values()
            ),
            "withdrawn_consents": sum(
                len([c for c in consents if c.withdrawn_at]) for consents in self._consents.values()
            ),
        }

        # Calculate breach metrics
        period_breaches = [
            b for b in self._breaches.values() if start_date <= b.created_at <= end_date
        ]
        breach_metrics = {
            "total_breaches": len(period_breaches),
            "critical_breaches": len(
                [b for b in period_breaches if b.severity == BreachSeverity.CRITICAL]
            ),
        ...
        report = ComplianceReport(
            ...
            high_risk_ai_systems=len(
                [
                    s
                    for s in self._ai_systems.values()
    ...
    def _calculate_avg_dsar_days(self, dsars: list[DataSubjectRequest]) -> float:
        ...
        total_days = sum((d.response_sent_at - d.received_at).days for d in completed)
    ...
    async def verify_control(
        ...
        if control.verification_function:
            ...
            if verify_fn:
                try:
                    automated_result = (
                        await verify_fn() if asyncio.iscoroutinefunction(verify_fn) else verify_fn()
                    )
                except Exception as e:
                    logger.error(
                        "verification_failed",
