...
class VendorRiskManagementService:
                        ...
                        expiring.append(
                            {
                                ...
                                "days_remaining": (
                                    contract.expiration_date - datetime.now(UTC)
                                ).days,
                            }
                        )
    ...
    def get_vendor_summary(self, vendor_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "assessments": {
                ...
                "last_assessment": recent_assessment.completed_at.isoformat()
                if recent_assessment
                else None,
                "last_score": recent_assessment.overall_score if recent_assessment else None,
                "next_due": vendor.next_assessment_due.isoformat()
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "by_category": {
                cat.value: len([v for v in vendors if v.category == cat]) for cat in VendorCategory
            },
            "assessments_overdue": len(self.get_overdue_assessments()),
            "contracts_expiring_30d": len(self.get_expiring_contracts(30)),
            "total_incidents": sum(len(self._incidents.get(v.vendor_id, [])) for v in vendors),
        }
