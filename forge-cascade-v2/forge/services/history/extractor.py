...
class HistoryExtractor:
    ...
    def _init_patterns(self) -> None:
        ...
        self._condition_keywords = [
            ...
            'stroke', 'seizure', 'epilepsy', 'asthma', 'copd',
            'depression', 'anxiety', 'dementia', 'alzheimer',
            'parkinson', 'arthritis', 'osteoporosis', 'thyroid',
        ]
    ...
    async def _parse_condition(
        ...
        age_match = re.search(r'age\s*(\d+)|(\d+)\s*years?\s*old|since\s*age\s*(\d+)', sentence, re.IGNORECASE)
        age_at_onset = None
        if age_match:
    ...
    def _extract_procedures(
        ...
        proc_keywords = [
            'surgery', 'operation', 'procedure', 'biopsy',
            'appendectomy', 'cholecystectomy', 'hysterectomy',
            'arthroscopy', 'colonoscopy', 'endoscopy',
            'cesarean', 'c-section', 'tonsillectomy',
        ]
                ...
                for sentence in self._get_sentences(proc_text):
                    if keyword in sentence.lower():
                        proc = ProcedureRecord(
                            ...
                            procedure_type="surgery" if "surgery" in keyword or keyword.endswith("ectomy") else "procedure",
                        )
    ...
    def _extract_allergies(
        ...
        allergen_keywords = [
            ...
            'peanut', 'egg', 'milk', 'soy', 'wheat', 'fish',
        ]
    ...
    def _split_sections(
        ...
        headers = [
            'problem list', 'diagnoses', 'medical history',
            'past medical history', 'pmh', 'family history',
            'fh', 'medications', 'current medications',
            'surgical history', 'past surgical history', 'psh',
            'procedures', 'allergies', 'social history',
        ]
