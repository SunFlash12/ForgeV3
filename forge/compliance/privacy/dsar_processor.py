    ...
    async def _discover_from_source(
        ...
        try:
            ...
            for category in source.data_categories:
                ...
                if category_data:
                    discovered.append(DiscoveredData(
                        source_id=source.source_id,
                        source_name=source.source_name,
                        data_category=category,
                        record_count=len(category_data) if isinstance(category_data, list) else 1,
                        sample_fields=list(category_data.keys()) if isinstance(category_data, dict) else [],
                        data=category_data,
            ...
            for exception in exceptions:
                if exception in self._erasure_exceptions:
                    result.retention_reasons.append(
                        self._erasure_exceptions[exception]
                    )
