...
class WalletInfo(BaseModel):
    ...
    wallet_type: str = Field(
        default="eoa",
        description="Type of wallet (eoa, tba, multisig)"
    )
...
class TokenInfo(BaseModel):
    ...
    total_supply: int = Field(
        ...
        description="Total token supply (1 billion standard)"
    )
    circulating_supply: int = Field(
        default=0,
    ...
    bonding_curve_progress: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Progress toward graduation (0.0 to 1.0)"
    )
    is_graduated: bool = Field(
        default=False,
        description="Whether token has graduated from bonding curve"
    )
    market_cap_virtual: float = Field(
        default=0.0,
        description="Market cap denominated in VIRTUAL"
    )
...
class RevenueRecord(BaseModel):
    ...
    source_entity_type: str = Field(
        description="Type of source (capsule, overlay, agent)"
    )
    beneficiary_addresses: list[str] = Field(
        default_factory=list,
        description="Addresses receiving revenue share"
    )
