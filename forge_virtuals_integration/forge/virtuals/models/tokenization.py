...
class TokenDistribution(BaseModel):
    ...
    - 5% liquidity pool
    """
    public_circulation_percent: float = Field(
        default=60.0,
        ge=0,
        le=100
    )
    ecosystem_treasury_percent: float = Field(
        default=35.0,
        ge=0,
        le=100
    )
    liquidity_pool_percent: float = Field(
        default=5.0,
        ge=0,
        le=100
    )
...
class RevenueShare(BaseModel):
    ...
    creator_share_percent: float = Field(
        ...
        ge=0,
        le=100,
        description="Share to original creator/owner"
    )
    contributor_share_percent: float = Field(
        default=20.0,
        ge=0,
        le=100,
        description="Share to contributors (via Contribution Vault)"
    )
    treasury_share_percent: float = Field(
        default=50.0,
        ge=0,
        le=100,
        description="Share to entity treasury for operations"
    )
    buyback_burn_percent: float = Field(
        default=50.0,
        ge=0,
        le=100,
        description="Percent of treasury used for buyback-burn"
    )
...
class TokenizationRequest(BaseModel):
    ...
    token_name: str = Field(
        max_length=64,
        description="Full name for the token"
    )
    token_symbol: str = Field(
        max_length=10,
        description="Ticker symbol (e.g., FRGCAP)"
    )
    ...
    enable_holder_governance: bool = Field(
        default=True,
        description="Allow token holders to vote on entity decisions"
    )
    ...
    primary_chain: str = Field(
        default="base",
        description="Primary chain for token deployment"
    )
...
class TokenHolderProposal(BaseModel):
    ...
    proposal_type: str = Field(
        description="parameter_change, treasury_allocation, etc."
    )
...
class BondingCurveContribution(BaseModel):
    ...
    tokens_received: float = Field(
        description="FERC20 placeholder tokens received"
    )
